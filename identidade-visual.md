## ANTES DE COMEÇAR: CONTEXTO DO PROJETO
	...
	•	Nome do projeto: [Gerador de Dinâmicas Cristãs]
	•	O que faz: [Aplicativo que gera dinâmicas cristãs ideais para encontros de igreja em segundos.]
	•	Para quem: [Líderes cristãos que conduzem encontros em igrejas e precisam de ideias práticas para engajar grupos, 20 – 55 anos]
	•	Sensação desejada: [Acolhedor, simples e inspirador]
	•	O que NÃO quer parecer: [Não queremos que pareça: PDF digitalizado, dashboard corporativo, sistema administrativo, aplicativo complexo, ferramenta técnica. Também evitar aparência de: template genérico, site de igreja antigo, design pesado ou confuso]
	•	Light mode, dark mode ou ambos: [LIGHT]
	•	Referência de energia (opcional): [simplicidade de apps bíblicos, leveza de apps de devocional, minimalismo moderno. Referências: Bible App (YouVersion) — simplicidade e clareza + Notion — organização limpa]
